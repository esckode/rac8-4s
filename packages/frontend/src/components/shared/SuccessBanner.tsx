/**
 * SuccessBanner - Success message display component
 *
 * Shows success message with dismiss button.
 * Accessible: role="status" for screen readers.
 */

import React from 'react'
import { Button } from './Button'
import '../../../styles/tokens.css'

export interface SuccessBannerProps {
  message: string
  onDismiss?: () => void
  className?: string
}

export const SuccessBanner: React.FC<SuccessBannerProps> = ({ message, onDismiss, className }) => {
  return (
    <div
      role="status"
      className={`
        flex
        items-center
        justify-between
        gap-[--s-4]
        bg-[--mint-100]
        border
        border-[--mint-200]
        rounded-[--r-md]
        px-[--s-4]
        py-[--s-3]
        ${className}
      `}
    >
      <div className="flex items-center gap-[--s-3]">
        <svg
          className="w-5 h-5 text-[--mint-600] flex-shrink-0"
          xmlns="http://www.w3.org/2000/svg"
          viewBox="0 0 20 20"
          fill="currentColor"
        >
          <path
            fillRule="evenodd"
            d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z"
            clipRule="evenodd"
          />
        </svg>
        <p className="text-sm font-medium text-[--mint-900]">{message}</p>
      </div>
      {onDismiss && (
        <Button
          variant="ghost"
          size="sm"
          onClick={onDismiss}
          className="flex-shrink-0"
          aria-label="Dismiss success message"
        >
          <svg
            className="w-4 h-4"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 20 20"
            fill="currentColor"
          >
            <path
              fillRule="evenodd"
              d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
              clipRule="evenodd"
            />
          </svg>
        </Button>
      )}
    </div>
  )
}
