  ...
  router.post('/:id/matches/:matchId/score', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const result = await submitScore(
        { db: deps.db, repo, groupRepo, conversationRepo, leaderboardRepo, jobQueue: deps.jobQueue, broadcastBus: deps.broadcastBus, config: deps.config },
        { tournamentId, matchId, playerId, score: req.body.score }
      )

      if (!result.ok) {
        return res.status(SCORE_ERROR_HTTP_STATUS[result.code]).json({ code: result.code, message: result.message })
      }
